...
OLAUNCHER_VERSION=1.6.93
AUTOOL_VERSION=0.1.0

finalname="MinecraftLauncher-$OLAUNCHER_VERSION-redist.jar"
...
if ! java -jar "jbsdiff/target/jbsdiff-1.0.jar" diff "../launcher.jar" "$basedir/minecraftlauncher/target/MinecraftLauncher-${OLAUNCHER_VERSION}.jar" "launcher.patch" || [ ! -e "launcher.patch" ]; then
...
finalhash=$(sha1sum "$basedir/minecraftlauncher/target/MinecraftLauncher-${OLAUNCHER_VERSION}.jar" | cut -d ' ' -f 1)
finalname=patched.jar
finalsz=$(du -b "$basedir/minecraftlauncher/target/MinecraftLauncher-${OLAUNCHER_VERSION}.jar" | cut -f 1)
interactive=true
#mainClass=
EOP
) > "patch.properties"
