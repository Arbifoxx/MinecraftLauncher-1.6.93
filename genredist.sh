...
MCLAUNCHER_VERSION=1.6.93
AUTOOL_VERSION=0.1.0

finalname="MinecraftLauncher-$MCLAUNCHER_VERSION-redist.jar"
...
if ! java -jar "jbsdiff/target/jbsdiff-1.0.jar" diff "../launcher.jar" "$basedir/olauncher/target/MinecraftLauncher-${MCLAUNCHER_VERSION}.jar" "launcher.patch" || [ ! -e "launcher.patch" ]; then
...
finalhash=$(sha1sum "$basedir/olauncher/target/MinecraftLauncher-${MCLAUNCHER_VERSION}.jar" | cut -d ' ' -f 1)
finalname=patched.jar
finalsz=$(du -b "$basedir/olauncher/target/MinecraftLauncher-${MCLAUNCHER_VERSION}.jar" | cut -f 1)
interactive=true
#mainClass=
EOP
) > "patch.properties"
